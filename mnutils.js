if (typeof setTimeout === "undefined") {
  ...
  globalThis.setTimeout = function(callback, delay) {
    ...
    NSTimer.scheduledTimerWithTimeInterval(ms / 1000.0, false, function () {
        // 执行回调
...
class MNUtil {
  ...
  static addErrorLog(error,source,info){
    ...
    // Runtime notification integration can be attached by the plugin layer.
  }
...
class MNConnection{
  ...
  /**
   * 对于得到的相应，通过res.ok判断是否成功，通过res.json()获取响应数据
   * @param {string} url 
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.ok) {
          ...
          this.addErrorLog("fetchIPInfo failed", "fetchIPInfo", JSON.stringify(res.asJSONObject(), null, 2))
          return undefined
        }
      }
    } catch (error) {
      MNUtil.addErrorLog(error, "fetchIPInfo")
      return undefined
    }
  }
...
static async readJSONFromWebDAV(config) {
  try {
    ...
    MNUtil.addErrorLog(error, "readJSONFromWebDAV")
    return undefined
  }
}
static async readJSONFromURL(url,headers = {}) {
  try {
    ...
    MNUtil.addErrorLog(error, "readJSONFromURL")
    return undefined
  }
}
/**
